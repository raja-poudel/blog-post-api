...
export default async (app: FastifyInstance, options: any, done: () => void) => {
  ...
  app.get("/all", { preValidation: [authenticateToken] }, handleGetAll);

  app.get<{
    Params: ParamSkipTake
  }>("/paginate/:skip/:take", { preValidation: [validateSkipTake, authenticateToken] }, handleGetByPaginate)

  app.get<{
    Params: ParamId
  }>("/:id", { preValidation: [validateId, authenticateToken] }, handleGetById);
