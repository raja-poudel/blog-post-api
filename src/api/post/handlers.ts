...
import { create, getAll, getByPaginate, getById, getByReferenceId, update, deletePostImages, deletePostThumbnail, deleteById } from './controllers';
...
export const handleGetByPaginate = async (request: any, reply: any) => {
  try {
    ...
    const { keyword, sort } = request.query;
    const { data, count } = await getByPaginate({ skip, take }, { keyword, sort });
    reply.status(200).send({
      statusCode: 200,
      count,
      data
...
export const handleGetReferenceId = async (request: any, reply: any) => {
  try {
    const id = Number(request.params.id)
    const response = await getByReferenceId({ id });
    reply.status(200).send({
      statusCode: 200,
      data: response
    })
