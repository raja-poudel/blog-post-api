...
import { getByReferenceId, create, getById, deleteById } from './controllers';


export const handleGetByReferenceId = async (request: any, reply: any) => {
  try {
    const referenceId = Number(request.params.referenceId)
    const response = await getByReferenceId({ referenceId })
    if (response === null) {
      reply.status(404).send(messages.doesNotExists)
    } else {
